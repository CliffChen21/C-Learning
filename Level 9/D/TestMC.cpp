...
boost::tuple<double, double> compute_sdse(vector<double> price_list, double r, double T)
{
	int M = price_list.size();
	double sum = 0; // sum of price
	double sum_sq = 0; // sum of price square

	for (int i = 0; i < price_list.size(); i++)
	{
		sum += price_list[i];
		sum_sq += price_list[i] * price_list[i];
	}

	// impplement the formula
	double tmp = sqrt(sum_sq - sum * sum / M);
	double sd = tmp * exp(-r * T) / (M - 1);
	double se = sd / sqrt(M);

	return boost::tuple<double, double>( se, sd );
}
...
int main()
{
	...
	boost::tuple<double, double> sdse = compute_sdse(price_list, myOption.r, myOption.T);
	...
	std::cout << "Standard Deviation: " << sdse.get<0>() << endl;
	std::cout << "Standard Error: " << sdse.get<1>() << endl;
